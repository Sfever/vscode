...
import { FileCoverage, TestCoverage, TestCoverageFileNode } from 'vs/workbench/contrib/testing/common/testCoverage';
...
export class TestCoverageView extends ViewPane {
	...
	protected override renderBody(container: HTMLElement): void {
		...
		this._register(autorun(reader => {
			...
			if (coverage) {
				this.tree.value ??= this.instantiationService.createInstance(TestCoverageTree, container, labels);
				this.tree.value.setInput(coverage);
			} else {
				this.tree.clear();
			}
		}));
...
class TestCoverageTree extends Disposable {
	private readonly tree: WorkbenchCompressibleAsyncDataTree<TestCoverage, CoverageTreeElement, void>;
	...
	constructor(
		...
		this.tree = <WorkbenchCompressibleAsyncDataTree<TestCoverage, CoverageTreeElement, void>>instantiationService.createInstance(
...
class TestCoverageDataSource implements IAsyncDataSource<TestCoverage, CoverageTreeElement> {
	public hasChildren(element: CoverageTreeElement | TestCoverage): boolean {
		return element instanceof TestCoverage || (isFileCoverage(element) && !!element.children?.size);
	}

	public async getChildren(element: CoverageTreeElement | TestCoverage): Promise<Iterable<CoverageTreeElement>> {
		if (element instanceof TestCoverage) {
			const tree = await element.getAllFiles();
			const files = [];
